...
const parseAliExpressSearchUrl: UrlParsingFunction = (url, loggingContext) =>
  parseUrlByStartAndSearchParam("https://www.aliexpress.com", "SearchText", url, loggingContext);

const parseAmazonSearchUrl: UrlParsingFunction = (url, loggingContext) =>
  parseUrlByStartAndSearchParam("https://www.amazon.com/s?", "k", url, loggingContext);
...
async function addTabsToNewGroup(tabIds: number[], newTabGroupName?: string) {
  const newTabGroupId = await chrome.tabs.group({ tabIds });

  if (newTabGroupName) chrome.tabGroups.update(newTabGroupId, { title: newTabGroupName });
}
...
async function closeCurrentGroup() {
  ...
  const idsOfTabsToRemove = tabsInTheSameGroup
    .map((tab) => tab.id)
    .filter((id): id is number => id !== undefined && id !== chrome.tabs.TAB_ID_NONE);
